...
export class JsonSchemaController {
  ...
  async upsertSchema(
    ...
    @RealmUpsertBody() req: Array<ContentUpsertReq>,
  ) {
    ...
    try {
      ...
      if (error.message.startsWith("strict"))
        throw new UnprocessableEntityException(error.message);
      if (error.message === "schema must be object or boolean")
        throw new UnprocessableEntityException(error.message);
    }
  ...
  async upsertSchemas(@RealmUpsertRealmBody() req: Array<RealmsUpsertReq>) {
    return await Promise.all(
      req.map(async ({ realm, contents }) =>
        this.upsertSchema(realm, contents),
      ),
    );
  }
  ...
  async getRealm(@QueryRealm() realm: string, @QueryIds() ids?: string[]) {
    ...
    if (!ids) {
      ...
      const data = reduceToContents(
        ...
        await this.schemaService.getRealm(realm),
      );
    ...
    const entities = await this.schemaService.getRealmContentByIds(
      realm,
      unmatchedKeys,
    );
    const count = await this.schemaService.countRealmContents();
  ...
  async deleteRealm(@ParamRealm() realm: string, @QueryIds() ids?: string[]) {
    ...
    const result = await this.schemaService.deleteRealmContentByIds(
      realm,
      filteredIds,
    );
    const count = await this.schemaService.countRealmContents();
