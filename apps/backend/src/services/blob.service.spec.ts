...
describe('BlobService', () => {
  ...
  describe('putObject', () => {
    it('should call minioClient.createBucketIfNotExists and minioClient.putObject with correct parameters', async () => {
      ...
      expect(mockMinioClient.createBucketIfNotExists).toHaveBeenCalledWith(bucketName);
      expect(mockMinioClient.putObject).toHaveBeenCalledWith(bucketName, objectName, stream, size, metaData);
    });
  });
});
