  ...
  return (
    <div className={style.pdfViewerMenu}>
      ...
      >
        <FaChevronLeft
          size={"1.3rem"}
          color={store.currentPage > 1 ? "unset" : "grey"}
        />
      </button>
      <p>
        {props.formatter?.(store.currentPage, store.pages) ??
          `Page ${store.currentPage} of ${store.pages}`}
      </p>
      <button
        disabled={store.currentPage >= store.pages}
        onClick={() => store.setCurrentPage(store.currentPage + 1)}
      >
        <FaChevronRight
          size={"1.3rem"}
          color={store.currentPage >= store.pages ? "gray" : "unset"}
        />
      </button>
    </div>
  );
