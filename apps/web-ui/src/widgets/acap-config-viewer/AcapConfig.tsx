    ...
    <div className={style.acapConfig}>
      ...
      >
        <AcapConfigItem label="Swagger">
          <SiSwagger size={'1.2rem'} color={getColor(acap?.data?.services.swagger, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        ...
        <AcapConfigItem label="GZIP">
          <FaCompress size={'1.2rem'} color={getColor(acap?.data?.services.gzip, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="EnvVars">
          <SiDotenv size={'1.2rem'} color={getColor(acap?.data?.services.resolveEnv, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
      </AcapConfigContainer>
      ...
      >
        <AcapConfigItem label="Redis">
          <SiRedis size={'1.2rem'} color={getColor(acap?.data?.databases.redis, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="MongoDB">
          <SiMongodb size={'1.2rem'} color={getColor(acap?.data?.databases.mongo, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="Minio">
          <SiMinio size={'1.2rem'} color={getColor(acap?.data?.databases.minio, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
      </AcapConfigContainer>

      <AcapConfigContainer
        style={{
          opacity: msBridge?.data?.brokers || acap?.data?.brokers?.useBullMQ ? '1' : '0.35',
        }}
        label="Brokers"
        color="orange"
        size="1.2rem"
      ...
      >
        <AcapConfigItem label="BullMQ">
          <SiSecurityscorecard
            size={'1.2rem'}
            color={getColor(
              msBridge?.data?.brokers.useBullMQ || acap?.data?.brokers?.useBullMQ,
              'yellowgreen',
              'crimson',
            )}
          />
        ...
        <AcapConfigItem label="MQTT">
          <SiMqtt size={'1.2rem'} color={getColor(msBridge?.data?.brokers.useMQTT, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="PubSub">
          <SiRedis size={'1.2rem'} color={getColor(msBridge?.data?.brokers.useRedisPubSub, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="RabbitMQ">
          <SiRabbitmq size={'1.2rem'} color={getColor(msBridge?.data?.brokers.useRabbitMQ, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
        <AcapConfigItem label="Kafka">
          <SiApachekafka size={'1.2rem'} color={getColor(msBridge?.data?.brokers.useKafka, 'yellowgreen', 'crimson')} />
        </AcapConfigItem>
      </AcapConfigContainer>
    </div>
  );
